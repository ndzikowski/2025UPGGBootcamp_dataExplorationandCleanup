...
The first half of this morning, we'll go through the package `Tidyverse` which you all should have
installed as part of the lesson yesterday.
...
generally also work on data frames and tibbles.
...
![](https://raw.githubusercontent.com/ndzikowski/2025UPGGBootcamp_dataExplorationandCleanup/main/images/blank_etal_2017.png)
...
![](https://raw.githubusercontent.com/ndzikowski/2025UPGGBootcamp_dataExplorationandCleanup/main/images/blanketal2017_fig2.png)

[Figure 2. Transcription of genes involved in ribosome biogenesis peaks in the G1 phase]
...
In addition to these, we will need help from 2 gene annotation files to understand the data: 1)
Ribosome biogenesis gene annotations. This has all yeast gene names and their corresponding
systematic IDs (This is a yeast thing, ok. Don't ask me) 2) Annotations (from Dryad) for all yeast
genes, not just the ones involved in ribosome biogenesis

### Who cares?

RNA sequencing has been fundamental to genomics research. It's very possible you might generate data
like this in your own research. You may be analying changing mRNA levels in the cell due to a normal
cellular process, or due to some genetic, epigenetic, or chemical perturbation you did in an
experiment.
...
We will store the file as a variable first for efficiency, then read in the line of each file using
the "read_lines" function. Feel free to load the file using the path as it appears on your local
machine; however, in case of any issues, a download link for the data is already provided for you in
...
Try "printing" the values contained in the variable `periodically_expressed_genes` and checking its
data type.
...
`periodically_expressed_genes` is a "vector" data type. It is similar to a 1D array in some other
programming languages, like Python.

`periodically_expressed_genes` is really simple & clean data. What about `mRNA_data`?

```{r}

```

This one is a dataframe (table) with more information, and it has a header at the top. Luckily, if
we turn the file into a tibble, that header will be recognized immediately.

```{r}

```

Remember, the output of many tidyverse "read" functions will automatically be a tibble! Applying
read_tsv to a data file can accomplish this.

Now that we've checked the formats of the experimental data, let's look at the annotations for
...
Here, we can see that there are comments at the top marked with a "!". This is really common in gene
annotation files, and we should skip them if we want a workable data frame.
...
What about for the Dryad annotations containing the systematic IDs/common gene names?
...
Here's a really easy, built-in way in R to do so:
...
But we're using tidyverse, so we don't even need `length()`. Just open the tibble, the information
is right there!
...
cerevisiae* - these can be found in the column 'Systematic Name/Complex Accession' in the
annotations. These IDs won't tell you a whole lot about the gene, so to understand the biology here
and communicate it to the public, it's better to have the common gene names. These can be found in
the 'Gene/Complex' column.
...
Still, using what we just learned, let's double check that to be sure.
...
The most useful part of this annotation file will be to convert the systematic gene IDs into more
common names for downstream analyses.
...
**Note:** There are often many ways format tidyverse functions. Choose the formatting that works
best for you. I like this one best:
...
We can also check if our favorite gene is on this list! It would be easy enough to do manually, or
with Ctrl+F in a text editor, but let's pretend our dataframe is still really long...and that our
text editor has crashed and doesn't wanna open again...
...
### Exercise: Check whether the gene "RPS6B" is a periodically expressed gene, and find its systematic name.
...
Let's read the Dryad annotation file!

The columns `"orf"` and `"gene"` will be useful to us for now (these correspond to the systematic ID
and common gene name, like in the other annotation file.)

Let's create a new data frame with only these columns:
...
This dataset actually has a lot more interesting information, like estimated mRNA and protein levels
for each gene. To explore these columns, we can use the `arrange()` function. This will allow us to
sort the dataframe according to a variable we're interested in.

```{r}

```

The default is ascending order. What goes at the end?

```{r}

```

### Exercise: arrange data in descending order of common gene name instead
...
Here's another interesting tidbit: with `arrange()`, NA values automatically get put the end,
regardless of whether you're arranging in ascending or descending order. This makes removal of NA
values/data exploration more convenient.
...
but not the 1998 study they used to validate their data? (Remember - only genes identified in BOTH
of those studies actually made it into the in `periodically_expressed_genes` data). What "new"
periodically expressed genes did this study identify?
...
We need to figure out a way to join these two datasets on a common column. We can try out a bunch of
`join` methods and see what we get.

A column that both datasets have in common is the "systematic ID". However, this column has a
different label in each dataset: "SystematicName" in the Dryad annotations, "ORF" in the mRNA
dataset.

### Exercise: Change the column name in one of these datasets so that they match.
...
key. If there is no corresponding value in "y", we'll see "NA".
...
```{r}
write_csv(mRNA_named, "../output/mRNA_data_with_gene_names.csv")
```

### Reshaping tibbles

Sometimes, we might want to reshape a tibble to make it better formatted for plotting/visualization
later on.

### Exercise: How can the mRNA data be reshaped? (Think about the "pivot" functions we learned earlier today). Once you reshape the data, discuss: What downstream analyses might require you to reshape the data like this?
...
Now I'm realizing, we might run into issues if all of the cell volumes are formatted as a "(integer)
fL", instead of just the integer.

### Exercise: What function(s) can we add on to remove the " fL" string from every value in the "Vol_fL" column?
...
Now, we can easily pick 3 genes we're most interested in, and examine (or plot) their fluctuating
mRNA levels over the course of the cell cycle.

Let's save that information into a small dataframe, and then it will be ready to plot.
...
```{r}
write_csv(mRNA_data_3genes, "../output/mRNA_data_3genes.csv")
```

### What else can we do with this data? What would YOU be interested in figuring out?
